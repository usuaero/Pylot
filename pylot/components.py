...
class LandingGear:
    ...
    def __init__(self, name, **kwargs):
        ...
        # Load params
        self.name = name
        self._unit_sys = kwargs.get("units")
        self._pos = import_value("tip_loc", kwargs, self._unit_sys, None)
        self._dir = import_value("shock_dir", kwargs, self._unit_sys, [0.0, 0.0, 1.0])
        self._k = import_value("shock_stiffness", kwargs, self._unit_sys, None)
        self._c = import_value("shock_damping", kwargs, self._unit_sys, None)
        self._u_f_roll = import_value("rolling_friction_coef", kwargs, self._unit_sys, 0.0)
        self._u_f_slid = import_value("sliding_friction_coef", kwargs, self._unit_sys, 0.0)


    def get_landing_FM(self, y):
